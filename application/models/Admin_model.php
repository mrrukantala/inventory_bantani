...
class Admin_model extends CI_Model
{
    ...
    public function getStock($limit = null, $id_barang = null, $range = null)
    {
        $this->db->select('b.id_barang,b.nama_barang, bm.supplier_id, s.nama_supplier, bm.tanggal_masuk, bk.tanggal_keluar, m.jumlah_minimal, m.jumlah_maximal, bm.jumlah_masuk, bk.jumlah_keluar, b.stok');
        $this->db->join('minmax m', 'm.barang_id = b.id_barang');
        $this->db->join('barang_keluar bk', 'bk.barang_id = b.id_barang');
        $this->db->join('barang_masuk bm', 'bm.barang_id = b.id_barang');
        $this->db->join('supplier s', 's.id_supplier = bm.supplier_id');
        if ($limit != null) {
            $this->db->limit($limit);
        }
        ...
        if ($range != null) {
            $this->db->where('bk.tanggal_masuk' . ' >=', $range['mulai']);
            $this->db->where('bk.tanggal_masuk' . ' <=', $range['akhir']);
        }
        $this->db->group_by('b.id_barang');

        $this->db->order_by('bm.id_barang_masuk, bk.id_barang_keluar', 'DESC');
        return $this->db->get('barang b')->result_array();
    }

    public function getMinmax($limit = null, $id_minmax = null, $range = null)
    {
        $this->db->select('*');
        ...
        $this->db->order_by('id_minmax', 'DESC');
        return $this->db->get('minmax m')->result_array();
    }

    public function getPengajuanBarang($limit = null, $id_pengajuan = null, $range = null)
    {
        $this->db->select('*');
        $this->db->join('jenis j', 'j.id_jenis = bp.jenis_id');
        $this->db->join('user u', 'u.id_user = bp.user_id');
        ...
        return $this->db->get('barang_pengajuan bp')->result_array();
    }

    public function getVerifikasiBarang($limit = null, $id_pengajuan = null, $range = null)
    {
        $this->db->select('*');
        $this->db->join('jenis j', 'j.id_jenis = bp.jenis_id');
        $this->db->join('user u', 'u.id_user = bp.user_id');
